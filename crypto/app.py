...
def load_data(currency_price_unit):
    ...
    df = pd.DataFrame(columns=['Name', 'Symbol', '1 hour change (%)', '24 hours change (%)', '7 days change (%)', 'Price', '24-hour volume'])
    df['Name'] = coin_name
...
def crypto():
    ...
    st.dataframe(df_change[['Name', 'Symbol', 'Price']], height=df_selected_coin.shape[0]*100)
    ...
    plot_settings = {'color': df_change[orders[percent_timeframe]].map(
        {
            True: 'g',
            False: 'r'
        }),
